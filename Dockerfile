...
# Use chess user and group for non-root environments
...
RUN wget http://prdownloads.sourceforge.net/sbcl/sbcl-1.5.5-x86-64-linux-binary.tar.bz2
RUN tar -xf sbcl-1.5.5-x86-64-linux-binary.tar.bz2
RUN rm sbcl-1.5.5-x86-64-linux-binary.tar.bz2
RUN cd sbcl-1.5.5-x86-64-linux && sh install.sh
RUN rm -r sbcl-1.5.5-x86-64-linux
